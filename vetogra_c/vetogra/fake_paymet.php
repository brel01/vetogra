                                  ...
                                  if($qq->rowCount() > 0){
                                    ...
                                    if($ref_id == $row_k['user_id']){
                                      ...
                                      if($new_eth_task_amt > $eth_task_amt){
                                        ...
                                        $a_qqqq->execute([$admin, $ref_id, $amt_paid, $admin_amt, $admin, $task_id, $date]);
                                        $error = "success1";
                                        ...
                                        $a_qqqq = $snake_dbh->prepare("INSERT INTO veto_gas(upline_usr_id, downline_usr_id, gas_amt, task_amt, upline_task_id, downline_task_id, date) VALUE (?,?,?,?,?,?,?)");
                                        $a_qqqq->execute([$admin, $user_id, $amt_paid, $admin_amt, $task_id, $admin, $date]);
                                        $error = "success2";
                                        $msg = json_encode($error);
                                        echo $error;
                                      }else{
                                        ...
                                        $a_qqqq = $snake_dbh->prepare("INSERT INTO veto_gas(upline_usr_id, downline_usr_id, gas_amt, task_amt, upline_task_id, downline_task_id, date) VALUE (?,?,?,?,?,?,?)");
                                        $a_qqqq->execute([$user_id, $admin, $amt_paid, $admin_amt, $task_id, $admin, $date]);
                                        $error ="success3";
                                        $msg = json_encode($error);
                                        echo $error;
                                      }
