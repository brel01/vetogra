  ...
  if($query){
    if($query->rowCount() == 1){
      ...
      if($row['task_payment_status'] == 0){
        $qry_t = $snake_dbh->prepare("SELECT eth_d_trans_id, eth_d_amount, eth_d_uid, eth_d_status, cancled FROM eth_deposit_trans  WHERE eth_d_status=0 && task_id=? && cancled=0 && invalid=0 && pending=0");
        ...
        if($qry_t){
              ...
              if($amount_paid >= $rw['eth_d_amount']){
                ...
                if($get_spill_query){
                  ...
                  $fectched_spill_amt = $fectched_spill_amt + $spill_amount;
                    $date = time();
                    $qry_u = $snake_dbh->prepare("UPDATE veto_tasks SET task_spillover_amt=?, task_payment_status=1, eth_deposite_id=?, pending=0 WHERE task_id=? && cancled=0 && task_payment_status=0");
                    $qry_u->execute([$fectched_spill_amt, $deposite_id, $task_id]);

                  if($qry_u){
                    $qry_y = $snake_dbh->prepare("UPDATE eth_deposit_trans SET eth_d_amount_paid=?, eth_d_status=1, eth_d_confirm_date=?, pending=0 WHERE eth_d_status=0 && task_id=? && cancled=0");
          ...
          $error = "Error occured during procesing41";
          $msg = json_encode($error);
          echo $error;
        }
      }else{
