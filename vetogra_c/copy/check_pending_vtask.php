...
if(isset($_POST)){
  if(isset($_SESSION['@snake_id'])){
    ...
    if($query->rowCount() > 0){
      $qry = $snake_dbh->prepare("SELECT eth_d_amount, eth_d_date, eth_d_address, qrcode_url, pending FROM eth_deposit_trans WHERE eth_d_uid=? && eth_d_status=0 && cancled=0");
      ...
      if($qry){
        if($qry->rowCount() == 1){
          ...
          $pending = $row['pending'];
          $data = ["msg"=>"success", "eth_amt"=>$eth_amt, "eth_date"=>$eth_date, "eth_add"=>$eth_add, "qrcode_url"=>$qrcode_url, "pending"=>$pending];
